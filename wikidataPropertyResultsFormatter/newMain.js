...
`;
        ...
        let output = await fetch(fullUrl, {headers}).then(body => body.json()).catch(err => {throw err})
        return output;
    }
}
...
async function processQueryData(data) {
    let output = "";
...
function queryForArgument(arg = "") {
    ...
    queryDispatcher.query("Q5")
    .then(dataJson => dataJson.results.bindings)
    .then(data => {
        let output = "";
    
        await processQueryData(data);
